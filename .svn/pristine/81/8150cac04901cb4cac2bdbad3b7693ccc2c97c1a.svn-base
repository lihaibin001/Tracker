...
{
//   return((!CAN_Get_Ignition_State())&&false);/*todo:other condition TBD,e.g.energy off*/
  return false;
}
...
static uint8_t ps_entry_user_on (void)
{
   ...
   if ((PS_Logical_Ignition_On() == ON))
   {
      ...
      {
//	     TMR_Start_Timer(PS_OBD_ON_TIMER, PS_ACC_ON_TIME, null_action);
      }
   }
   else
   {
      ...
      if(PS_Get_Obd_On_Minutes() != 0xFFFF)
      {
              if((Sys_Is_Gsensor_Wakeup())
                  ||(Sys_Is_Low_Batt_Wakeup()))
              {
//                        TMR_Start_Timer(PS_OBD_ON_TIMER, PS_EVENT_ON_TIME, null_action);
              }
              else
              {
//                        TMR_Start_Timer(PS_OBD_ON_TIMER, PS_OBD_ON_TIME, null_action);
      ...
      {
//	     TMR_Stop_Timer(PS_OBD_ON_TIMER);/*timed-play mode is disabled*/
      }
   }      
   return(PS_USER_ON);
}
...
//   OS_Send_Message(OS_IOT_TASK, Build_Message(TM_EVT_ENG_ON_OFF, ON));	 
   return(PS_ENG_ON);
}
