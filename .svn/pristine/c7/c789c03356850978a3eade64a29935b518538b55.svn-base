...
int main(void)
{
    ...
    {
        /* Read sw_infor from SPI FLASH memory */
        sFLASH_ReadBuffer((uint8_t *)&sw_infor, NOR_FLASH_OTA_Addr, sizeof(SW_infor_t));
        if(sw_infor.ota_ctrl!=0x03)
        {
            uint8_t sw_start_flag=Dev_App_Started();
            switch (sw_start_flag)
            {
                case 0:
                    // do not know if app can start, set flag
                    Dev_Set_upd_flag();
                    JumpToApp();
                    break;
                case 1:
