...
static void prvGSM_evt_sim_upload(int16_t data);
...
#if 0
    if(Sys_Is_Low_Batt_Wakeup())
    {
        OS_Send_Message(OS_IOT_TASK,Build_Message(TM_EVT_BATTERY_FAULT, 1));
        DEBUG_PRINT0 (DEBUG_MEDIUM,"[2G]:===Battery Is Low=== \r\n");
    }

    OS_Sleep(MSec_To_Ticks(10000)); //Sleep 1s for module startup and simcard communication.
#endif
...
static void prvGSM_evt_car_theft(int16_t data)
{
    ...
    if (NET_DATA_TRANS == vATProt_Get_TCP_State())
    {
        vTelmApp_uploadData(TELM_SEC_EVT_REMOVAL, NULL);
    }
    else
    {
        vTelmApp_Backup_Msg(TELM_SEC_EVT_REMOVAL);
    }
}
...
static void prvGSM_evt_sim_upload(int16_t data)
{
    ...
    if (NET_DATA_TRANS == vATProt_Get_TCP_State())
    {
	    vTelmApp_uploadData(TELM_SEC_EVT_SIM_INFO, NULL);
    }
}
