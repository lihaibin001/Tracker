...
static uint8_t default_config[]={0x00,0x01,
                                ...
                                'c','m','n','e','t',0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
                                0x01,0x0A,0x00,
...
bool Set_Config(Config_t config)
{
    ...
    OS_Sleep(100);
    sFLASH_ReadBuffer(config.byte, FLASH_CONFIG_OFFSET, sizeof(Config_t));
    if(memcmp(config.byte, tmp_config.byte, sizeof(GPS_Info_t))==0)
        return true ;
    count++;
...
//DEBUG_PRINT0(DEBUG_MEDIUM, "[PARAM]:GPS Info Data initialized!\n\r");
}
...
//        sFLASH_ReadBuffer(tmp_g_gps_info.byte, FLASH_GPS_INFO_OFFSET, sizeof(GPS_Info_t));

//        if(0==(memcmp(gps_info_t->byte, tmp_g_gps_info.byte, sizeof(GPS_Info_t))))
//            return true;
//        count++;
    }
    return true;
...
{
//    memset(gps_log_t->byte , 0 , sizeof(GPS_log_t)) ;
    ...
    */
    ...
    if(g_gps_info.log_sector_write_ptr >= GPS_LOG_NUMBER_PER_SECOTR)
    {
        ...
        //DEBUG_PRINT0(DEBUG_LOW,"[GPS LOG] set write pointer to 0 , sector + 1\r\n");
    }
    if(g_gps_info.log_write_sector >= GPS_LOG_MAX_SECTOR_NUM)
    {
        g_gps_info.log_write_sector = 0;
        g_gps_info.log_sector_write_ptr = 0;
        //DEBUG_PRINT0(DEBUG_LOW,"[GPS LOG] set write pointer to 0 , sector = 0\r\n");
    }
...
/*
    if(g_gps_info.log_sector_read_ptr >= GPS_LOG_NUMBER_PER_SECOTR)
    {
        g_gps_info.log_sector_read_ptr = 0;
        g_gps_info.log_read_sector ++;
        DEBUG_PRINT0(DEBUG_LOW,"[GPS LOG] set read pointer to 0 , sector + 1\r\n");
    }
    if(g_gps_info.log_read_sector >= GPS_LOG_MAX_SECTOR_NUM)
    {
        g_gps_info.log_read_sector = 0;
        g_gps_info.log_sector_read_ptr = 0;
        DEBUG_PRINT0(DEBUG_LOW,"[GPS LOG] set read pointer to 0 , sector = 0\r\n");
    }*/
       ...
       (g_gps_info.log_sector_read_ptr == g_gps_info.log_sector_write_ptr))
    {
        ...
        g_gps_info.log_sector_write_ptr = 0;
        Init_GPS_Info();
//        DEBUG_PRINT0(DEBUG_MEDIUM,"[GPS LOG] reset read pointer ,write pointer and total number!\r\n");
    }
...
bool Load_Param(void)
{
    ...
    {
        res = false ;
/*        if(((g_devinfo_sct.structData.initialized[0] << 8) + \
            g_devinfo_sct.structData.initialized[1]) != INITIALIZE_DATA)
            Init_Param_In_Flash();
        else
            Init_Manufacture_Setting();*/
    }
...
// length not greater than 100
void test_flash(uint32_t address, uint8_t *test_flash_data, uint16_t test_flash_len)
{
    uint8_t flash_test_result=1;
    uint32_t test_flash_address=address;
    uint8_t test_flash_read[100];
    uint8_t i=0;

    if (test_flash_len > 100)
    {
        DEBUG_PRINT0(DEBUG_MEDIUM,"[DIAG]Flash Test data length exceed max range\n\r");
    }
    if (sFLASH_S25FL116K_ID!=sFLASH_ReadID())
    {
    ...
    {
        diag_set_flash_result(1);
    }
    else
    {
        diag_set_flash_result(0);
    }
}
