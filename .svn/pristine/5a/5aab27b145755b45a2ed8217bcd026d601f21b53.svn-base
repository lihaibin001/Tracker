...
static void vATApp_Removal_Alert(void)
{
    OS_Send_Message(OS_IOT_TASK,Build_Message(TM_EVT_CAR_THEFT, 1));
}
...
void vATApp_Loop_Check(void)
{
    ...
    switch(net_state)
    {
        ...
        case NET_POWER_ON:
            ...
            vATProt_sendAT_Command(AT_CMD_IPD_HEAD_SET, NULL, NULL);
            vATProt_sendAT_Command(AT_CMD_SET_CREG, NULL, NULL);
            vATProt_sendAT_Command(AT_CMD_SYNC_RTC, NULL, NULL);
        ...
        case NET_REGISTERED:
            ...
            vATProt_sendAT_Command(AT_CMD_CCLK, NULL, NULL);
            set_gps_upload_backup(GPS_BACKUP_STATE_PUSH);
            vATProt_Set_TCP_State(NET_GPRS_ATTACHED);
            break;
        case NET_GPRS_ATTACHED:
            //Register APN
            vATProt_sendAT_Command(AT_CMD_QIREGAPP,NULL,NULL);
            vATProt_sendAT_Command(AT_CMD_GPRS_QUERY,NULL,NULL);
            break;
        case NET_APN_REGISTERED:
            vATProt_sendAT_Command(AT_CMD_QILOCIP, NULL, NULL);
            vATProt_sendAT_Command(AT_CMD_QIOPEN, NULL, NULL);
            break;
        case NET_TCP_CONNECTED:
            /* Check if message need resend */
            OS_Send_Message(OS_DIAG_TASK,Build_Message(DIAG_EVT_CONNECTED, 1));
            Conn_Timeout = 0;
            vTelmApp_uploadData(TELM_SEC_EVT_DEV, NULL);
            if (GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_0) == 1)
            {
                // wait previous message sent
                TMR_Start_Timer(TELM_UPLOAD_TIMER, 2000, vATApp_Removal_Alert);
            }
...
//    vATProt_sendAT_Command(AT_CMD_IP_SEND_DATA_EXCUTE, NULL, NULL);
    vATProt_sendAT_Command(AT_CMD_QISEND, NULL, NULL);
}
