...
/*		if((batt_volt_max - batt_volt_min) >=100)
		{
		}*/
...
void periodic_128x(void)             /* Called every  512    / 640 ms        */
{
    if (vGps_Get_Gps_Status())
    {
        ...
        if (gps_first_fixed_flag > 0)
        {
            ...
            OS_Send_Message(OS_RECORD_TASK,Build_Message(RECORD_EVT_FIX, 1));
        }
...
{
//    gps_data_t gps_data;
//    DEBUG_PRINT2(DEBUG_MEDIUM,"[2G]:Net status:%d, upload status = %d\n\r",vATProt_Get_TCP_State(),eTelmApp_upload_state());
//    vGps_Get_Gps_Info(&gps_data);
//    DEBUG_PRINT2(DEBUG_MEDIUM,"[GPS]:long: %x, lat: %x\n\r", *((uint32_t *)gps_data.longitude), *((uint32_t *)gps_data.latitude));
}
...
static void Periodic_Set_GPS_Fixed(uint32_t count)
{
    gps_first_fixed_sec=(count*64)/100;
}
