/**********************************************************************
   ...
   Module Description         : GSensor Module.
 ...
 *********************************************************************/
static uint8_t prvGSensor_readRegs(uint8_t regAddr, uint8_t* data, uint8_t len);
static void prvGSensor_writeRegs(uint8_t regAddr, uint8_t data, uint8_t NumByteToWrite);

static void FXLS1_FIFO_DataRead(void);
static void GSensor_LowLevel_Init(void);
static void Gsensor_FIFO_Fuction_Disable(void);
static void Gsensor_VECM_Theshold_Config(uint16_t threshold);

#if 0
static void Gsensor_FFMT_fuction_init(void);
static void Gsensor_TRANS_fuction_init(void);
static void Gsensor_CLICK_fuction_init(void);
#endif
/**********************************************************************
...
void GSensor_init(void)
{
	...
	if(Cold_Start())
	{
		...
		prvGSensor_readRegs(WHO_AM_I_REG,&deviceWhoAmI,1);
		if (deviceWhoAmI != WHO_AM_I_VAL)
		{
			DEBUG_PRINT0(DEBUG_HIGH,"[Gsensor]:Failed to get ID of Gsensor \r\n");
		}

		/*!!!Modification of control register can only occur in Standby mode */ 
		prvGSensor_readRegs(CTRL_REG1,&readdata,1);
		prvGSensor_writeRegs(CTRL_REG1, readdata & ~ACTIVE_MASK,1);

		uDelay(200);//delay 200us 
		/*Check if we are in STANDBY mode using SYSMOD*/
		prvGSensor_readRegs(SYSMOD_REG,&sysmod,1);
		if ((sysmod & SYSMOD_MASK) != SYSMOD_STANDBY)
		{
			DEBUG_PRINT1 (DEBUG_HIGH,"[Gsensor]:Entering stby mode failed, current mode is %x\r\n",sysmod);
		}

		/*Enable low power in sleep state, high resolution in active state*/
		prvGSensor_writeRegs(CTRL_REG2, SMOD_LOW_POWER|MOD_HIGH_RES,1);

		/*Set full-scale range to +- 4G, ¡À0.488 mg/LSB.*/
		prvGSensor_writeRegs(XYZ_DATA_CFG_REG,FULL_SCALE_4G,1);

               /*VECM interrupt Threshold level 1,~25mg */
		Gsensor_VECM_Theshold_Config(G_SENSOR_LEVEL_1_VECM_THRESHOLD);
		
		/*Enable A_VECM detection interrupts*/
		prvGSensor_writeRegs(CTRL_REG4, INT_EN_A_VECM_MASK,1);

		/*Route A_VECM interrupts to INT1,INT2 is unused*/
		prvGSensor_writeRegs(CTRL_REG5, INT_CFG_A_VECM_MASK,1);
		...
		prvGSensor_writeRegs(CTRL_REG1, DATA_RATE_100HZ|ACTIVE_MASK,1);
...
/*******************************************************************************
*    Function: Gsensor_FIFO_Fuction_Disable
*
*  Parameters: None
*     Returns:  None
...
static void Gsensor_FIFO_Fuction_Disable(void)
{
	...
	/*Disable FIFO*/
	prvGSensor_writeRegs(F_SETUP_REG,0x00,1); 
...
/*******************************************************************************
*    Function: Gsensor_VECM_fuction_Config
*
*  Parameters: None
*     Returns:  None
...
static void Gsensor_VECM_Theshold_Config(uint16_t threshold)
{
	...
	uint8_t readdata;
	#endif
	prvGSensor_writeRegs (A_VECM_THS_MSB, (uint8_t)(threshold>>8),1); 
	prvGSensor_writeRegs (A_VECM_THS_LSB, (uint8_t)(threshold),1); 
	// set debounce to 2, A_VECM_CNT = 2 /400hz = 5ms
	prvGSensor_writeRegs (A_VECM_CNT, 0x02,1);
	// A_VECM_CFG 
	//	0110 1000 = 0x68
	...
	prvGSensor_writeRegs (A_VECM_CFG, 0x48,1);

	
	#if (DEBUG_LVL == DEBUG_MEDIUM)
	prvGSensor_readRegs(A_VECM_THS_MSB,&readdata,1);
	DEBUG_PRINT1 (DEBUG_MEDIUM,"[Gsensor]:VECM MSB = %x\r\n",readdata);
	prvGSensor_readRegs(A_VECM_THS_LSB,&readdata,1);
	DEBUG_PRINT1 (DEBUG_MEDIUM,"[Gsensor]:VECM LSB = %x\r\n",readdata);
	#endif
}
/*******************************************************************************
*    Function: GSensor_gotoSleep
*
*  Parameters:  uint8_t regAddr
*     Returns:  None
* Description:  read register of Gsensor through IIC bus
*******************************************************************************/
void GSensor_gotoSleep(void)
{
	uint8_t readdata;
	uint8_t sysmod = 0;
	uint8_t intSource;
	...
	prvGSensor_writeRegs(F_SETUP_REG, F_MODE_TRIGGER|0x0A,1); 
	/*Set the FIFO interrupt Trigger for VECM Detection*/
	prvGSensor_writeRegs(TRIG_CFG_REG, TRIG_A_VECM_MASK,1); 

	/*Config the VECM trigger thresold in sleep mode*/
	Gsensor_VECM_Theshold_Config(G_SENSOR_LEVEL_2_VECM_THRESHOLD); 

       /*Enable FIFO interrupt detection interrupts*/
	prvGSensor_writeRegs (CTRL_REG4, INT_EN_FIFO_MASK,1);

	/*route FIFO interrupt to INT1;INT2 is unused*/
	prvGSensor_writeRegs (CTRL_REG5, (INT_CFG_FIFO_MASK),1);

	/*Enable A_VECM as a wake-up source*/
	//prvGSensor_writeRegs (CTRL_REG3, FIFO_GATE_MASK|WAKE_A_VECM_MASK,1);
	...
	prvGSensor_readRegs(INT_SOURCE_REG,&intSource,1); 
	if (GSENSOR_INT1_ACTIVE () != 0X01)
	{
		DEBUG_PRINT0 (DEBUG_HIGH,"[Gsensor]:Failed to config Gsensor before sleep\r\n");
	}
}

/*******************************************************************************
*    Function: Gsensor_Clear_Crank_Count
*
*  Parameters:  
*     Returns:  None
* Description:  
*******************************************************************************/
void Gsensor_Clear_Crank_Count(void)
{
	crank_count = 0;
}

/*******************************************************************************
*    Function: Gsensor_Get_Crank_Count
*
*  Parameters:  
*     Returns:  None
* Description:  
*******************************************************************************/
uint16_t Gsensor_Get_Crank_Count(void)
{
	return crank_count;
}

/*******************************************************************************
*    Function: Gsensor_Clear_Shake_Count
*
*  Parameters:  
*     Returns:  None
* Description:  
*******************************************************************************/
void Gsensor_Clear_Shake_Count(void)
{
	shake_count = 0;
}

/*******************************************************************************
*    Function: Gsensor_Get_Shake_Count
*
*  Parameters:  
*     Returns:  None
* Description:  
*******************************************************************************/
uint16_t Gsensor_Get_Shake_Count(void)
{
    return shake_count;
}
...
#ifdef CONFIG_G_SENSOR_UPLOAD
/*******************************************************************************
*    Function: Gsensor_check_data
*
*  Parameters:  
*     Returns:  None
* Description:  
*******************************************************************************/
void Gsensor_check_data(void)
{
...
* Description:  
*******************************************************************************/
void GSensor_Read_Xyz_Value(void)
{
	FXLS1_sensorData_t sensorData;
	// read accel registers
	prvGSensor_readRegs(OUT_X_MSB_REG, (uint8_t*) &sensorData,6);
...
//        DEBUG_PRINT3(DEBUG_MEDIUM,"[GSensor]: [X %d,Y %d,Z %d]\r\n",xAccel_mg,yAccel_mg,zAccel_mg);
}
...
static void FXLS1_FIFO_DataRead(void)
{
	...
	prvGSensor_readRegs(F_STATUS_REG, (uint8_t*) &readvalue,1);
	if(( readvalue & 0xE0) >= 0x60) // FIFO trigger event is dectected and all 32 samples ready?
	{
		/*read FIFO buffer*/
		prvGSensor_readRegs(OUT_X_MSB_REG, (uint8_t*) FIFO_sensorDate,192);
		#if (DEBUG_LVL == DEBUG_LOW)
		// process and align data
		for(i=0;i<32;i++)
		{
	...
	Gsensor_FIFO_Fuction_Disable();
}

void GSensor_getFIFO(uint8_t *data)
{
    memcpy(data,FIFO_sensorDate,192);
}

#ifdef CONFIG_G_SENSOR_UPLOAD
void GSensor_getHighG(uint8_t *data)
{
    uint8_t i=0;
...
#endif
/*******************************************************************************
*    Function: GSensor_Interrupt_Handler
*
*  Parameters:None
*     Returns:  None
* Description: 
...
void GSensor_Interrupt_Handler(void)
{
	...
	if (GSENSOR_INT1_ACTIVE () == 0x00)//gsensor interrupt occur
	{
		...
		prvGSensor_readRegs(INT_SOURCE_REG,&readbyte,1);
//		DEBUG_PRINT1 (DEBUG_MEDIUM,"[Gsensor]:Interrupt source = %x\r\n",readbyte);
		...
		if ((readbyte & SRC_A_VECM_MASK) == SRC_A_VECM_MASK)
		{
			...
			DEBUG_PRINT1 (DEBUG_LOW,"[Gsensor]:Crank Count = %d\r\n",crank_count);
		}
...
/*******************************************************************************
*    Function: prvGSensor_readRegs
*
*  Parameters:  uint8_t regAddr
*     Returns:  result of reading:
* Description:  read regitor of Gsensor through IIC bus
*******************************************************************************/
static uint8_t prvGSensor_readRegs(uint8_t regAddr, uint8_t* data, uint8_t len)
{
	uint8_t vol;
...
/*******************************************************************************
*    Function: prvGSensor_writeRegs
*
*  Parameters:  uint8_t regAddr
*     Returns:  None
* Description:  write regitor of Gsensor through IIC bus
*******************************************************************************/
//static bool 
static void prvGSensor_writeRegs(uint8_t regAddr, uint8_t data, uint8_t NumByteToWrite)
{
