...
#define DEFAULT_APN "cmiot"
#define DEFAULT_DOMAIN_NAME "obu1.deren.com"

#define UNICOM_SMS_NUM "106550010646"
...
typedef enum AT_Sms_Parse_Mode_tag
{
    SMS_PARSE_BYTE=0,
    SMS_PARSE_WORD,
}AT_Sms_Parse_Mode_t;
...
static uint8_t connect_fail_count = 0;
...
static uint8_t sms_parse_mode=SMS_PARSE_BYTE;
...
static const AT_Cmd_Info_t AT_Cmd_Info_table[AT_CMD_NUM] =
{
    ...
    { 1,  "",               EXCUTE,   MSec_To_Ticks(100),  TRUE},  /* AT_CMD_SEND_POST_DATA */
    { 5,  "AT+CGACT=",      EXCUTE,   MSec_To_Ticks(2000),  TRUE},  /* AT_CMD_ACTIVE_PDP_CONTEXT */
    { 5,  "AT+CGATT=",      EXCUTE,   MSec_To_Ticks(2000),  TRUE},  /* AT_CMD_DISATT_3G */
    ...
    { 1,  "AT+CIPSTART=",     EXCUTE,   MSec_To_Ticks(2000), TRUE},  /* AT_CMD_IP_OPEN_EXCUTE */
    ...
    { 1,  "AT+QIOPEN=",    EXCUTE,    MSec_To_Ticks(5000),  TRUE},  /* AT_CMD_QIOPEN */
    ...
    { 3,  "AT+CGACT=1",     EXCUTE,   MSec_To_Ticks(300),  TRUE},  /* AT_CMD_ACTIVE_PDP */
    { 3,  "AT+CGATT=1",     EXCUTE,   MSec_To_Ticks(300),  TRUE},  /* AT_CMD_ATTACH_GPRS */

    { 3,  "AT+QISTAT",     QUERY,   MSec_To_Ticks(300),  TRUE},  /* AT_CMD_QISTAT */
    { 3,  "AT+QIACT",     EXCUTE,   MSec_To_Ticks(300),  TRUE},  /* AT_CMD_QIACT */
    { 3,  "AT+CGREG?",     QUERY,   MSec_To_Ticks(300),  TRUE},  /* AT_CMD_GPRS_QUERY */
    { 3,  "AT+QILOCIP",     QUERY,   MSec_To_Ticks(300),  TRUE},  /* AT_CMD_QILOCIP */
    { 3,  "AT+QBAND?",     EXCUTE,   MSec_To_Ticks(300),  TRUE},  /* AT_CMD_QBAND */
};
...
static const AT_Cmd_Encode_Fun ATProt_encode_table[] =
{
    ...
    NULL,    /* AT_CMD_CCLK */
    NULL,    /* AT_CMD_ACTIVE_PDP */
    NULL,    /* AT_CMD_ATTACH_GPRS */

    NULL,    /* AT_CMD_QISTAT */
    NULL,    /* AT_CMD_QIACT */
    NULL,    /* AT_CMD_GPRS_QUERY */
    NULL,    /* AT_CMD_QILOCIP */
    NULL,    /* AT_CMD_QBAND */
};
...
extern void vATProt_Power_On(void)
{
    ...
    IO_GSM_PWR_ON_OUT(Bit_SET); //LOW for 1.1s, this value is used for M26
...
{
    IO_GSM_PWR_ON_OUT(Bit_SET); //LOW for 0.8s, this value is used for M26
...
char const * pcATProt_getImsiData(void)
{
    ...
    return "461234561231234";//Set IMSI as 461234561231234 for test.
#else
...
/*            if (TCP_RXING == prvATProt_Get_TCP_Mode())
        ...
        case START_RX_DATA:
            if (rx_char_count == 8)
            {
                ...
                if (strncmp(tcom_rx_buffer,"IPD",3)==0)
                {
                    for (i=0;i<8;i++)
                    {
                        if(*(tcom_rx_buffer+i)==':')
...
static void prvATProt_Resp_Common_Decode(char* tcom_rx_buffer, uint8_t rx_count)
{
...
#if 0
    DEBUG_PRINT1(DEBUG_MEDIUM,"[2G]%s\n\r",tcom_rx_buffer);
#endif
    ...
    if(!prvATProt_URC_Decode(tcom_rx_buffer, rx_count))
    {
        ...
        if (AT_pending_tx_cmd == AT_CMD_QIOPEN)
...
*/
        ...
        else if(!strncmp((char*)tcom_rx_buffer, ERROR_String, strlen(ERROR_String)))
        {
            ...
            else
            {
                ...
                {
                    // todo: Reset module and connect again
                    error_count = 0;
                    vATProt_Power_Off();
                    vATProt_Com_Reset();
                    vATProt_Set_TCP_State(NET_INIT);
		...
		else if (!strncmp((char*)tcom_rx_buffer, REMOTE_CLOSE_String, strlen(REMOTE_CLOSE_String)))
		{
            ...
            rl_delay_without_schedule(2000);
            TelmApp_Set_Server_Ack_State(false);
		...
		else if (!strncmp((char*)tcom_rx_buffer, OK_String, strlen(OK_String)))
		{
            ...
            if (AT_pending_tx_cmd == AT_CMD_QIACT)
            {
                vATProt_Set_TCP_State(NET_APN_REGISTERED);
            }
		}
...
static void prvATProt_URC_decode_CGreg(char* rx_buffer, uint8_t rx_count)
{
    if ((rx_count > 8) && ((rx_buffer[10] == '1')||(rx_buffer[10] == '5')))
    {
...
static void prvATProt_URC_decode_Signal(char* rx_buffer, uint8_t rx_count)
{
    ...
    DEBUG_PRINT1(DEBUG_MEDIUM,"[GPRS]CSQ:%d\n\r",net_Info.rssi);
}
...
static void prvATProt_URC_decode_Gprs(char* rx_buffer, uint8_t rx_count)
{
    ...
    if (state == 1)
    {
        ...
        if (NET_REGISTERED == vATProt_Get_TCP_State())
...
static void prvATProt_URC_decode_Sms(char* rx_buffer, uint8_t rx_count)
{
    ...
    uint8_t i=0;
    while((rx_buffer[i] != CHAR_COMMA) &&(i < rx_count))
    {
        i++;
    }
    if (i==rx_count)
        return;
    if (strncmp(UNICOM_SMS_NUM, &rx_buffer[i+2],sizeof(UNICOM_SMS_NUM)-1) == 0)
    {
        sms_parse_mode=SMS_PARSE_WORD;
    }
    else
    {
        sms_parse_mode=SMS_PARSE_BYTE;
    }
...
static void prvATProt_URC_decode_Clock(char* rx_buffer, uint8_t rx_count)
{
    ...
    // If device RTC still starts with 1970, initialize clock
    if (100000 > RTC_GetCounter())
    {
        uint16_t utc_year = (((clock_ref.year[0] - '0') * 1000) +
                      ((clock_ref.year[1] - '0') * 100) +
                      ((clock_ref.year[2] - '0') * 10) +
                      (clock_ref.year[3] - '0'));
        uint8_t utc_month = (((clock_ref.month[0] - '0') * 10) +
                      (clock_ref.month[1] - '0'));
        uint8_t utc_day = (((clock_ref.day[0] - '0') * 10) +
                      (clock_ref.day[1] - '0'));
        uint8_t utc_h = (((clock_ref.hour[0] - '0') * 10) +
                      (clock_ref.hour[1] - '0'));
        uint8_t utc_m = (((clock_ref.min[0] - '0') * 10) +
                      (clock_ref.min[1] - '0'));
        uint8_t utc_s = (((clock_ref.sec[0] - '0') * 10) +
                      (clock_ref.sec[1] - '0'));
...
static void prvATProt_parse_sms(char* rx_buffer, uint8_t rx_count)
{
    ...
    uint8_t data_len=0;
    uint8_t i=0;
    if (sms_parse_mode==SMS_PARSE_BYTE)
    {
    ...
    {
        uint8_t tmp_data_2[100];
        uint8_t tmp_data_len=0;
        for (i=0;(i*4)<rx_count;i++)
        {
            tmp_data_2[i]=StrtoHex(rx_buffer+(i*4)+2);
            tmp_data_len++;
        }
        for (i=0;(i*2)<tmp_data_len;i++)
        {
            tmp_data[i]=StrtoHex(tmp_data_2+(i*2));
            data_len++;
        }
    }
...
#ifdef USE_DOMAIN_NAME
    strncpy(AT_Cmd_Param.IP_Open_Setting.Dest_IP,DEFAULT_DOMAIN_NAME,sizeof(DEFAULT_DOMAIN_NAME));
#else
    get_server_ip();
#endif
    get_server_port();
...
void ATProt_Get_Imsi(uint8_t *imsi)
{
    ...
    for(;i<8;i++)
    {
        imsi_tmp[i]=StrtoDec(&net_Info.imsi[i*2],1)<<4;
        if (i<7)
        {
            imsi_tmp[i]|=StrtoDec(&net_Info.imsi[i*2+1],1);
        }
    }
    memcpy(imsi, imsi_tmp, 8);
}
